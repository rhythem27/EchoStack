...
- [ ] Implement a lightweight image pixel-difference / SSIM similarity comparator in WebAssembly or Canvas API:
...
- [ ] Wrap canvas frame capture loop in React frontend with `shouldSendFrame(currentCanvas)` filter.
- [ ] Log frame skip vs frame send metrics in client dev console (e.g. *"Skipped 82% identical static frames"*).
- [ ] Verify continuous smooth AI vision understanding when user shows dynamic motion vs static objects.

---

## 3. Verification Criteria
- [ ] Static camera feed streams only 1 initial frame and skips redundant frames until motion occurs.
- [ ] WebSocket traffic drops significantly while maintaining instant visual response on physical movement.
